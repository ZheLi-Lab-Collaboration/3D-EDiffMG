...
if __name__ == '__main__':
    ...
    for n, datas in enumerate(tqdm(data_list)):
        with torch.no_grad():
            ...
            logger.info("The %dth stable:%.4f" % (n + 1, stable / valid))
            logger.info("The %dth Uniq:%.4f" % (n + 1, len(set(smile_list)) / valid))
            print('----------------------------')
    ...
    print("Final stable:", stable / valid)
    print("Final unique:", len(set(smile_list)) / valid)
    print(len(set(smile_list)) / valid)
    logger.info("Final validity:%.4f" % (valid / num_samples))
    logger.info("Final stable:%.4f" % (stable / valid))
    logger.info("Final unique:%.4f" % (len(set(smile_list)) / valid))
