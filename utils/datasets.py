...
class ZINC3D(InMemoryDataset):

    def __init__(self, split, root='./data/ZINC', transform=None, pre_transform=None, pre_filter=None):
        self.split = split
        self.folder = osp.join(root, 'zinc')
        self.transform = transform

        super(ZINC3D, self).__init__(self.folder, transform, pre_transform, pre_filter)

        self.processed_paths[0] = './data/ZINC.zinc'
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return 'zinc_pyg_%s.pt' % self.split
...
def accumulate_grad_from_subgraph(model, atom_type, pos, bond_index, bond_type, batch, atom2res, batch_size=8,
    ...
    2. evaluate subgraphs using trained EDiffMG
