...
import time
from .dis_angle_cal_utils import \
    compute_bond_lengths, \
    compute_angle, \
    filter_angle_non_covalent_bonds_and_check_cyclopropyl, \
    filter_angle_length, \
    filter_bonds_by_length, \
    filter_real_non_covalent_edges, \
    filter_global_section_mask
...
def _extend_to_radius_graph(pos,
                            edge_index,
                            edge_type,
                            cutoff,
                            batch,
    ...
    # bond_lengths = compute_bond_lengths(pos, edge_index)
    # Step 1: filter length
    # filter_length_edge_index = filter_bonds_by_length(bond_lengths, edge_index, min_length=0.8, max_length=cutoff)
    filter_length_edge_index = radius_graph(pos, r=cutoff, batch=batch)

    # Step 2: filter angle


    filter_angle_non_covalent_edges, cyclopropyl_edges = filter_angle_non_covalent_bonds_and_check_cyclopropyl(pos, filter_length_edge_index, batch)

    filter_unique_angle_non_covalent_edges = torch.unique(filter_angle_non_covalent_edges, dim=0)
    cyclopropyl_uniqe_edges = torch.unique(cyclopropyl_edges, dim=0)


    if filter_unique_angle_non_covalent_edges.numel() > 0:
        filter_length_angle_edges = filter_angle_length(filter_length_edge_index, filter_unique_angle_non_covalent_edges)
    else:
        filter_length_angle_edges = filter_length_edge_index



    if cyclopropyl_uniqe_edges.numel() > 0:
        global_edge_section = filter_real_non_covalent_edges(filter_unique_angle_non_covalent_edges, cyclopropyl_uniqe_edges[:, 1:])

        # filter Covalent bond
        final_edge_index = torch.cat([filter_length_angle_edges, cyclopropyl_uniqe_edges[:, 1:]], dim=0)
    else:

        final_edge_index = filter_length_angle_edges
        global_edge_section = filter_unique_angle_non_covalent_edges

    "_______________________________________________________________________________"
    final_edge_index_t = torch.unique(final_edge_index, dim=0)
    final_unique_edge_index = final_edge_index_t.t()

    if final_unique_edge_index.size(0) != 2:
        final_unique_edge_index = final_unique_edge_index.t()

    final_unique_edge_index_ = torch.sparse.LongTensor(
        final_unique_edge_index,
        torch.ones(final_unique_edge_index.size(1)).long().to(pos.device) * unspecified_type_number,
        torch.Size([N, N]),
    )

    composed_adj = (bgraph_adj + final_unique_edge_index_).coalesce()  # Sparse (N, N, T)

    new_edge_index = composed_adj.indices()
    new_edge_type = composed_adj.values().long()


    if global_edge_section.numel() > 0:
        global_edge_section_mask = filter_global_section_mask(new_edge_index, global_edge_section)
    else:
        global_edge_section_mask = torch.empty((0), dtype=torch.bool, device=pos.device)

    return new_edge_index, new_edge_type, global_edge_section_mask.unsqueeze(1)
