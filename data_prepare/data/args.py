...
def setup_shared_args(parser):
    ...
    parser.add_argument('--lr-decay-type', type=str, default='cos', metavar='str',
                        help='Type of learning rate decay. '
                             '(cos | linear | exponential | pow | restart) (default: cos)')
    ...
    parser.add_argument('--sgd-restart', type=int, default=-1, metavar='int',
                        help='Restart SGD optimizer every (lr_decay)^p epochs, '
                             'where p=sgd_restart. (-1 to disable) (default: -1)')
    ...
    parser.add_argument('--loadfile', type=str, default='',
                        help='Set checkpoint file to load. '
                             'Leave empty to auto-generate from prefix. (default: (empty))')
    # Filename to save model checkpoint to
    parser.add_argument('--checkfile', type=str, default='',
                        help='Set checkpoint file to save checkpoints to. '
                             'Leave empty to auto-generate from prefix. (default: (empty))')
    # Filename to best model checkpoint to
    parser.add_argument('--bestfile', type=str, default='',
                        help='Set checkpoint file to best model to. '
                             'Leave empty to auto-generate from prefix. (default: (empty))')
    # Filename to save logging information to
    parser.add_argument('--logfile', type=str, default='',
                        help='Duplicate logging.info output to logfile. '
                             'Set to empty string to generate from prefix. (default: (empty))')
    ...
    # Directory to place saved EDiffMG
    ...
    parser.add_argument('--edge-cat', action='store_true',
                        help='Concatenate the scalars from different \ell '
                             'in the dot-product-matrix part of the edge network.')
