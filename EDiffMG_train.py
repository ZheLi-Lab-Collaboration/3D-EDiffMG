...
from EDiffMG.model import get_model
from utils.common import get_optimizer, get_scheduler
from utils.datasets import QM93D, ZINC3D
...
if __name__ == '__main__':
    ...
    if args.cuda:
        ...
        device = 'cpu'
    args.dataset = 'zinc' if 'zinc' in args.config else 'qm9'
    ...
    if resume:
        ...
        if not os.path.exists(os.path.join(log_dir, 'EDiffMG')):
            shutil.copytree('EDiffMG', os.path.join(log_dir, 'EDiffMG'))
    ...
    shutil.copyfile('EDiffMG_train.py', os.path.join(log_dir, 'EDiffMG_train.py'))
    ...
    #     train_set = Geom(pre_transform=transforms)
        ...
        train_set = ZINC3D('train', pre_transform=transforms)
        val_set = ZINC3D('valid', pre_transform=transforms)
        val_loader = DataLoader(val_set, config.train.batch_size, shuffle=False)
    else:
    ...
    patience = 50  # Set patience, for example, 10 epochs without improvement
    ...
    for it in range(start_iter, config.train.max_iters + 1):
        ...
        if avg_val_loss < best_val_loss:
            ...
            no_improvement_counter = 0  # Reset counter if validation loss improves
        ...
        if no_improvement_counter >= patience:
            print(f'Early stopping triggered after {it} iterations.')
